...
if __name__ == "__main__" :
    ...
    Sb = 0;
    for i in range(K) :
        DCl = DPPCA[:,L==i];    # take only samples of the class-i
        DClC = DCl - toCol(DCl.mean(1));    # center the data
        MC = toCol(DCl.mean(1)) - toCol(DPPCA.mean(1)); # center the mean of class, respect the global mean
    ...
    ## PROJECTING DATA ON NEW BASE ##
    DPLDAO = numpy.dot(WO.T,DPPCA);
    DPLDA = numpy.dot(W.T,DPPCA);
    ## PLOTTING THE DATA ##
